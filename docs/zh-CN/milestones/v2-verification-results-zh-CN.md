...
- MySQL 真实知识列表接入后的 API 完整测试：在 `services/api` 运行 `mvn test`
  - 结果：14 个测试通过，0 个失败。
- Redis 运行时状态接线后的 Collector 完整测试：在 `services/collector` 运行 `python -m pytest`
  - 结果：22 个测试通过，0 个失败。
- Qdrant 优先检索切换后的 AI worker 完整测试：在 `services/ai-worker` 运行 `python -m pytest`
  - 结果：21 个测试通过，0 个失败。
...
- 运维复核、告警和审计界面已查询持久化 V2 表，不再只返回硬编码数据。
- 用户、会话、情报、付费情报和知识的 API store 已使用 JDBC 持久化仓库。
- `GET /api/knowledge` 现已提供 MySQL 支持的知识列表契约，用于真实知识同步与验证。
- API、AI worker、Web 和数据库 seed 修正路径中的用户可见诊断文本已为可读 UTF-8 中文。
- Web 登录、诊断提交、报告元数据、付费情报和运维指标已通过真实 API client 调用。
- 未登录 Web 用户现在只看到独立登录界面；已登录工作台支持页面内中英文切换和退出登录状态清理。
- Collector 韧性辅助能力现已使用 Redis 支持的运行时状态，承载增量指纹去重和近期快照兜底。
- AI 检索现已将向量写入 Qdrant 并以 Qdrant 作为首选检索路径，同时保留 Python 侧的地域、行业、权益和质量重排；自检仍可阻止存疑冲突结论。

## 当前环境未执行

- Docker Compose 启动和 MySQL/Redis/Qdrant 健康检查。
  - 原因：当前这次验证环境中未提供 Docker CLI。
...
- 浏览器内真实登录手工流程。
  - 原因：本次验证期间未启动本地 API 服务和完整基础设施栈。
  - 待运行命令：启动 API 服务，在 `apps/web` 运行 `npm run dev`，然后在浏览器验证 `operator/password` 登录、语言切换、诊断提交和退出登录。
  - 补救步骤：在下一次本地完整栈冒烟验证中执行。

## 当前退出状态

V2 仍处于实施中，尚不能退出。MySQL 支持的 API 知识列表、Redis 支持的 collector 运行时状态、Qdrant 优先的 AI 检索路径以及现有 Web 登录门禁/双语界面检查已在当前代码库通过；恢复演练、压测、Docker 健康检查、浏览器内完整栈冒烟验证和灰度稳定性证据仍未完成。
