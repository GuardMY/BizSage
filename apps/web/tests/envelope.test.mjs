...
test("Admin page renders a dedicated operations workspace with real action handlers", async () => {
  ...
  assert.match(source, /saveAdminKnowledgeDraft/);
  assert.match(source, /submitAdminKnowledgeReview/);
  assert.match(source, /approveAdminKnowledgeReview/);
...
test("Admin V3 API client exposes real knowledge management endpoints for Admin-V3-3", async () => {
  const source = readFileSync(new URL("../lib/api-client.ts", import.meta.url), "utf8");
  assert.match(source, /fetchAdminKnowledgeNodes/);
  assert.match(source, /\/admin\/knowledge\/nodes/);
  assert.match(source, /fetchAdminKnowledgeDetail/);
  assert.match(source, /saveAdminKnowledgeDraft/);
  assert.match(source, /submitAdminKnowledgeReview/);
  assert.match(source, /approveAdminKnowledgeReview/);
  assert.match(source, /publishAdminKnowledgeVersion/);
  assert.match(source, /rollbackAdminKnowledgeNode/);
  assert.match(source, /fetchAdminKnowledgeDiff/);
  assert.match(source, /\/admin\/knowledge\/diff/);
});

test("Admin workspace styles provide a dedicated knowledge-management layout", async () => {
  const source = readFileSync(new URL("../app/globals.css", import.meta.url), "utf8");
  assert.match(source, /\.adminKnowledgeShell/);
  assert.match(source, /\.knowledgeNodeList/);
  assert.match(source, /\.knowledgeVersionItem/);
  assert.match(source, /\.knowledgeDiffGrid/);
});


test("Admin V3 API client exposes real collection management endpoints for Admin-V3-4", async () => {
  const source = readFileSync(new URL("../lib/api-client.ts", import.meta.url), "utf8");
  assert.match(source, /fetchAdminCollectionSources/);
  assert.match(source, /fetchAdminCollectionSourceDetail/);
  assert.match(source, /saveAdminCollectionSource/);
  assert.match(source, /saveAdminCollectionKeyword/);
  assert.match(source, /runAdminCollectionSource/);
  assert.match(source, /fetchAdminCollectionJobs/);
  assert.match(source, /fetchAdminCollectionDeadLetters/);
  assert.match(source, /\/admin\/collection\/sources/);
  assert.match(source, /\/admin\/collection\/jobs/);
  assert.match(source, /\/admin\/collection\/dead-letters/);
});

test("Admin page wires the collection workspace for Admin-V3-4", async () => {
  const source = readFileSync(new URL("../app/admin/page.tsx", import.meta.url), "utf8");
  assert.match(source, /CollectionWorkspace/);
  assert.match(source, /fetchAdminCollectionSources/);
  assert.match(source, /fetchAdminCollectionSourceDetail/);
  assert.match(source, /saveAdminCollectionSource/);
  assert.match(source, /saveAdminCollectionKeyword/);
  assert.match(source, /runAdminCollectionSource/);
  assert.match(source, /setActiveSection\("collection"\)/);
});
